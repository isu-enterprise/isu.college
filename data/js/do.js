...
// console.log(data);
    ...
    showUserIdentityInput: function(e) {
      ...
      document.body.insertAdjacentHTML('beforeend', '<aside id="user-identity-input" class="do on"><button class="close" title="Close">вќЊ</button><h2>Sign in with WebID</h2><label>HTTP(S) IRI</label> <input id="webid" type="text" placeholder="http://csarven.ca/#i" value="" name="webid"/> <button class="signin">Sign in</button></aside>');
...
// console.log(i);
    ...
    showDocumentInfo: function() {
      document.body.insertAdjacentHTML('beforeend', '<menu id="document-menu" class="do hidden hide" hide><button class="show" title="Open Menu"><i class="fa fa-bars"></i></button><header></header><div></div><footer><dl><dt>About</dt><dd id="about-dokieli"><i class="fa fa-github"></i> <a href="https://github.com/linkeddata/dokieli">dokieli</a></dd></dl></footer></menu>');
      ...
      if (stylesheets.length > 1) {
        ...
        for (var i = 0; i < stylesheets.length; i++) {
          ...
          if(stylesheet.matches('[rel~="alternate"]')) {
            s += '<li><button title="Change to вЂ' + view + 'вЂ™ view">' + view + '</button></li>';
          }
          else {
    ...
    showEmbedData: function(node) {
      ...
      var eventEmbedData = function(e) {
        ...
        var embedMenu = '<aside id="embed-data-entry" class="do on tabs"><button class="close" title="Close">вќЊ</button>\n\
        <h2>Embed Data</h2>\n\
    ...
    showToC: function() {
      ...
      if (sections.length > 0) {
        ...
        s = '<aside id="toc" class="do on' + sortable + '"><button class="close" title="Close">вќЊ</button></aside>';
        ...
        ids[i].addEventListener('mouseenter', function(e){
          ...
          if (!fragment && e.target.parentNode.nodeName.toLowerCase() != 'aside'){
            ...
            switch(e.target.nodeName.toLowerCase()) {
              default:    sign = 'рџ”—'; break;
              case 'section':
                switch (e.target.id) {
                  default:          sign = 'В§'; break;
                  case 'references':    sign = 'в›'; break;
                  case 'acknowledgements':  sign = 'вє'; break;
                  case 'results':       sign = 'в€ћ'; break;
                  case 'related-work':    sign = 'вЊ'; break;
                  case 'keywords':      sign = 'рџЏ·'; break;
                  case 'conclusions':     sign = 'в€ґ'; break;
                }
                break;
              case 'aside':   sign = 'вЂ '; break;
              case 'p':     sign = 'В¶'; break;
              case 'pre':   sign = 'рџ–©'; break;
              case 'nav':   sign = 'в›'; break;
              case 'dfn':   sign = 'рџ“‡'; break;
              case 'table':   sign = 'рќ„њ'; break;
              case 'figure':  sign = 'вќ¦'; break;
              case 'img':   sign = 'рџ–ј'; break;
              case 'video':   sign = 'рџЋћ'; break;
              case 'audio':   sign = 'рџ”Љ'; break;
              case 'footer':  sign = 'вё™'; break;
            }
...
';
    ...
    shareResource: function(e, iri) {
      ...
      if(!(DO.C.User.Graph && ((DO.C.User.Knows && DO.C.User.Knows.length > 0) || (DO.C.User.Graph.owlsameAs && DO.C.User.Graph.owlsameAs._array.length > 0)))) {
        ...
        noContactsText = '<p>No contacts with <i class="fa fa-inbox"></i> Inboxes found. Acquire <i class="fa fa-thermometer-empty"></i> cool friendsвЂЅ</p><p>Optionally enter targets individually:</p>';
      }
        ...
        DO.U.getContacts(url).then(
          function(contacts) {
            if(contacts.length > 0) {
              ...
              e.target.parentNode.innerHTML = 'No contacts with <i class="fa fa-inbox"></i> Inboxes found. Acquire <i class="fa fa-thermometer-empty"></i> cool friendsвЂЅ</p><p>Optionally enter targets individually:</p>';
            }
          },
          function(reason) {
             console.log(reason);
...
// console.log(iri.toString());
    ...
    showResourceBrowser: function() {
      ...
      var browserHTML = '<aside id="resource-browser" class="do on"><button class="close" title="Close">вќЊ</button><h2>Resource Browser</h2></aside>';
    ...
    openDocument: function(e) {
      ...
      document.body.insertAdjacentHTML('beforeend', '<aside id="open-document" class="do on"><button class="close" title="Close">вќЊ</button><h2>Open Document</h2></aside>');
...
// console.log(template)
    ...
    createNewDocument: function(e) {
      e.target.disabled = true;
      document.body.insertAdjacentHTML('beforeend', '<aside id="create-new-document" class="do on"><button class="close" title="Close">вќЊ</button><h2>Create New Document</h2></aside>');
...
// console.log(baseURLSelectionChecked);
    ...
    saveAsDocument: function(e) {
      e.target.disabled = true;
      document.body.insertAdjacentHTML('beforeend', '<aside id="save-as-document" class="do on"><button class="close" title="Close">вќЊ</button><h2>Save As Document</h2></aside>');
    ...
    viewSource: function(e) {
      e.target.disabled = true;
      document.body.insertAdjacentHTML('beforeend', '<aside id="source-view" class="do on"><button class="close" title="Close">вќЊ</button><h2>Source</h2><textarea id="source-edit" rows="24" cols="80"></textarea><p><button class="create">Update</button></p></aside>');
...
WHERE {\n\
  ...
  ?observation ?propertyMeasure ?obsValue .\n\
    ...
    getReferenceLabel: function(motivatedBy) {
      ...
      switch(motivatedBy) {
        default: break;
        case 'oa:assessing':  s = 'вњЄ'; break;
        case 'oa:commenting': s = 'рџ—Ё'; break;
        case 'oa:bookmark':   s = 'рџ”–'; break;
        case 'oa:replying':   s = 'рџ’¬'; break;
        case 'oa:describing': s = 'вЂ»'; break;
      }
    ...
    //    _mediumEditors[1].destroy();
      ...
      enableEditor: function(editorMode, e, selector) {
        ...
        if (!document.getElementById('document-editor')) {
          document.body.insertAdjacentHTML('beforeend', '<aside id="document-editor" class="do hidden hide" hide></aside>');
        }
